...
export class MapSearchController{
    ...
    public getPhotoForPlaces:Function = (req:express.Request,res:express.Response) => {
        ...
        Promise.all(place_ids.map(async id=>{
            return await axios.get(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${id}&fields=geometry,vicinity,price_level,formatted_address,adr_address,name,rating,photo,formatted_phone_number,website,url,opening_hours&language=ko&key=${this.GOOGLE_API}`)
